...
// const PRIVATE_APP_ACCESS = process.env.HUBSPOT_PRIVATE_APP_TOKEN;
const CUSTOM_OBJECT = process.env.CUSTOM_OBJECT; 
...
app.get("/", async (req, res) => {
    const url = `https://api.hubapi.com/crm/v3/objects/${CUSTOM_OBJECT}?properties=name&properties=description&properties=category`;
    const headers = {
        Authorization: `Bearer ${PRIVATE_APP_ACCESS}`,
        "Content-Type": "application/json",
    };
    ...
    try {
        ...
        res.render("homepage", { title: "Homepage", data: resp.data.results || [] });
    } catch (error) {
        console.error(
            "Error fetching custom object data:",
...
// ROUTE 2 - Form page for creating new record
